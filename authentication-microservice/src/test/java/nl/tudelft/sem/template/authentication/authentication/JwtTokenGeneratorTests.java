...
public class JwtTokenGeneratorTests {
    ...
    public void setup() throws NoSuchFieldException, IllegalAccessException {
        ...
        user = new User(netId, "someHash", new ArrayList<GrantedAuthority>(
                Collections.singletonList(new SimpleGrantedAuthority(EmployeeType.CANDIDATE.toString()))));
    }

    @Test
    public void generatedTokenHasCorrectClaims() {
        // Act
        String token = jwtTokenGenerator.generateToken(user);

        // Assert
        Claims claims = getClaims(token);
        assertThat(claims.get("role")).isEqualTo(user.getAuthorities().iterator().next().toString());
    }
